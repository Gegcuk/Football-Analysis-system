...
class Tracker:
    ...
    def get_object_tracks(self, frames, read_from_stub=False, stub_path=None):
        ...
        tracks = {
            "players": [],  # {0:{"bbox":[0, 0, 0, 0], 1:{"bbox":[0,0,0,0],}, 3:{"bbox":[0, 0, 0, 0]}}
            "referees": [],
            "ball": [],
        }
        ...
        for frame_num, detection in enumerate(detections):
            ...
            # To sv.Detection format
            detection_supervision = sv.Detections.from_ultralytics(detection)

            # Convert gp to player
            for object_index, class_id in enumerate(detection_supervision.class_id):
                if cls_names[class_id] == "goalkeeper":
                    detection_supervision.class_id[object_index] = cls_names_inv[
                        "player"
                    ]
    ...
    def draw_triangle(self, frame, bbox, color, track_id=None):
        ...
        triangle_points = np.array([[x, y], [x - 10, y - 15], [x + 10, y - 15]])
    ...
    def draw_team_ball_control(self, frame, frame_num, team_ball_control):
        ...
        cv2.putText(frame, f"Team 1 ball control: {team1 * 100:.0f}%", (1400, 900), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
        cv2.putText(frame, f"Team 2 ball control: {team2 * 100:.0f}%", (1400, 950), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 0), 3)
        return frame
